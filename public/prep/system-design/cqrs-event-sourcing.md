...
```visualizer
{
  ...
  "content": "graph TD\n    Client-->|Command: BookRide| API_CMD[Command API]\n    API_CMD-->|Validate| Domain[Domain Logic]\n    Domain-->|Save Event| ES[(Event Store DB)]\n    ES-->|Publish| Bus{Event Bus}\n    \n    Bus-->|RideBooked| Projector[Projector Service]\n    Projector-->|Update View| ReadDB[(Read DB / Cache)]\n    \n    Client-->|Query: GetRideStatus| API_QUERY[Query API]\n    API_QUERY-->|Fast Fetch| ReadDB"
}
```
